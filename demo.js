...
/**
 * Lens402 - Monetized Transaction History API Interactive Demo
 *
...
async function demoPaymentFlow() {
  ...
  await typeWriter('\n🎭 Welcome to the Lens402 Interactive Demo!', 'green', 40);
  ...
  print('\n$ curl http://localhost:3000/api/transfers?address=64swuFWG5RDSc3SDUhRKefYJtY5q1EkoixcPg3ZsVpcG', 'yellow');
  await sleep(1200);
  ...
  // Step 3: Payment
  printStep(3, 'User sends payment on Solana network');
  await sleep(800);
  ...
  print('    http://localhost:3000/api/transfers?address=64swuFWG5RDSc3SDUhRKefYJtY5q1EkoixcPg3ZsVpcG', 'yellow');
  await sleep(1200);
  ...
  printBox('Transaction History Data', [
    '',
    '📊 Found 3 transfers for C5J2Hm5...96045',
    '',
  ], 'green');
...
async function showQueryExamples() {
  ...
  await sleep(1000);
  print('\n   💡 Returns: All SOL, tokens, and NFT transfers', 'cyan');

  await sleep(1500);

  print('\n📚 Example 2: Get Only Token (SPL) Transfers', 'bright');
  await sleep(300);
  print('   Use case: Track token movements (USDC, DAI, etc.)\n', 'dim');
  print('   $ curl -H "X-Payment-Hash: demo" \\', 'yellow');
  print('       "http://localhost:3000/api/transfers?address=C5J2Hm5...&category=SPL"', 'yellow');

  await sleep(1000);
  print('\n   💡 Returns: Only SPL token transfers', 'cyan');
  ...
  print('   $ curl -H "X-Payment-Hash: demo" \\', 'yellow');
  print('       "http://localhost:3000/api/transfers?address=C5J2Hm5...&category=NFT,nft"', 'yellow');

  await sleep(1000);
  print('\n   💡 Returns: Only NFT transfers (NFT + PDA)', 'cyan');
  ...
  print('   $ curl -H "X-Payment-Hash: demo" \\', 'yellow');
  print('       "http://localhost:3000/api/transfers?address=C5J2Hm5...&maxCount=10"', 'yellow');
  ...
  print('   $ curl -H "X-Payment-Hash: demo" \\', 'yellow');
  print('       "http://localhost:3000/api/transfers?address=C5J2Hm5...', 'yellow');
  ...
  print('   $ curl -H "X-Payment-Hash: demo" \\', 'yellow');
  print('       "http://localhost:3000/api/transfers?fromAddress=C5J2Hm5..."', 'yellow');
  ...
  print('   $ curl -H "X-Payment-Hash: demo" \\', 'yellow');
  print('       "http://localhost:3000/api/transfers?toAddress=C5J2Hm5..."', 'yellow');
  ...
  print('   category         - external,internal,SPL,NFT,PDA', 'white');
...
async function showAIAgentScenario() {
  ...
  const agentSteps = [
    ...
    { emoji: '📡', text: 'Requests Lens402: "Get latest 20 transfers"', color: 'yellow' },
    { emoji: '💸', text: 'Lens402 responds: 402 Payment Required', color: 'red' },
    { emoji: '⚡', text: 'Agent auto-pays: 0.01 USDC → Lens402', color: 'blue' },
    { emoji: '✅', text: 'Payment confirmed in 2 seconds', color: 'green' },
    { emoji: '📊', text: 'Lens402 returns: 20 latest transactions', color: 'green' },
...
async function showAgentCommerceScenario() {
  ...
  await typeWriter('\n🤝 Scenario: Lens402 serves data to Analytics Agent autonomously\n', 'cyan', 25);

  await sleep(800);

  print('\n┌─────────────────────┐         ┌─────────────────────┐', 'magenta');
  print('│   Analytics Agent   │         │    Lens402 Service     │', 'magenta');
  ...
  const commerceSteps = [
    ...
    {
      agent: 'Analytics Agent',
      action: 'Discovers Lens402 API (via on-chain registry)',
      icon: '🔍',
      color: 'cyan'
    },
    {
      agent: 'Analytics Agent',
      action: 'Requests: GET /api/transfers?address=addr...',
      icon: '📡',
      color: 'yellow'
    },
    {
    ...
    {
      agent: 'Analytics Agent',
      action: 'Sends: 0.01 USDC on Solana to Lens402',
      icon: '💸',
      color: 'blue'
    },
    {
  ...
  await sleep(300);
  print('     • Data vending agents (like Lens402)', 'white');
  ...
  print('\n   Trading Bot → Pays Lens402 → Gets wallet history →', 'yellow');
...
async function showSampleResponse() {
  console.clear();
  printBox('Sample API Response', ['GET /api/transfers?address=C5J2Hm5...'], 'cyan');
...
async function explainX402() {
  console.clear();
  printBox('Understanding x402 Protocol', ['HTTP-Solanad micropayments for the internet'], 'magenta');
...
async function showQuickStart() {
  console.clear();
  printBox('Quick Start Guide', ['Get Lens402 running in 3 steps'], 'green');
...
async function main() {
  while (true) {
    ...
    switch (choice) {
      ...
      case '8':
        console.clear();
        print('\n👋 Thanks for checking out Lens402! 🎰\n', 'green');
        print('Star us on GitHub: https://github.com/Lens402/lens402-sdk', 'cyan');
