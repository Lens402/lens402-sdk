...
const CONFIG_DIR = path.join(os.homedir(), '.Lens402');
...
export async function configCommand(options) {
  ...
  if (options.set) {
    ...
    if (Object.keys(config).length === 0) {
      ...
      console.log(chalk.white('  defaultNetwork - Default network (e.g., Solana-Devnet)'));
      console.log(chalk.white('  paymentAddress - Default payment address\n'));
      console.log(chalk.gray('Example:\n'));
      console.log(chalk.cyan('  Lens402 config --set defaultUrl=http://localhost:3000\n'));
    } else {
