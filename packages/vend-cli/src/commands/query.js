...
export async function queryCommand(url, options) {
  ...
  if (options.wallet) {
    try {
      ...
      console.error(chalk.yellow('Private key must be in format: addr...\n'));
      process.exit(1);
    }
  }
  ...
  const { txHash } = await inquirer.prompt([
    {
      ...
      validate: (input) => {
        ...
        if (!input.startsWith('addr')) return 'Transaction hash must start with addr';
...
function displayTransfersTable(transfers) {
  ...
  displayTransfers.forEach(transfer => {
    table.push([
      ...
      chalk.white(transfer.asset || 'SOL'),
