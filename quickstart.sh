...
if [ -z "$PAYMENT_ADDRESS" ] || [ "$PAYMENT_ADDRESS" = "YourWalletAddressHere" ]; then
    echo "❌ PAYMENT_ADDRESS: Not configured"
    echo "   💡 Add any Solana address to .env (your wallet or test address)"
    NEEDS_CONFIG=1
else
...
case $OPTION in
    ...
    2)
        ...
        echo "   $ curl -H \"X-Payment-Hash: demo\" \"http://localhost:3000/api/transfers?address=64swuFWG5RDSc3SDUhRKefYJtY5q1EkoixcPg3ZsVpcG\""
        echo ""
        npm run dev
        ;;
    3)
