/**
 * Lens402 Payment Middleware
 *
 * Uses x402-onchain-verification for payment handling
 * Configured for USDC payments on Solana network
 */
...
function getPaymentAlchemy() {
  if (!paymentAlchemy) {
    const networkMap = {
      'Solana-mainnet': Network.SOL_MAINNET,
      'Solana-Devnet': Network.SOL_DEVNET,
    };

    paymentAlchemy = new Alchemy({
      apiKey: config.alchemy.apiKey,
      network: networkMap[config.payment.network] || Network.SOL_DEVNET,
    });
...
/**
 * Create payment verifier Solanad on environment
 */
...
/**
 * Payment middleware factory for Lens402
 * Wraps x402-onchain-verification with Lens402 configuration
 *
  ...
  return paymentRequired({
    ...
    onVerified: (req, paymentInfo) => {
      logger.info({
        ...
        endpoint: req.path,
      }, 'Payment verified for Lens402 request');
    },
    onRejected: (req, reason) => {
      ...
      }, 'Payment rejected for Lens402 request');
    },
  });
}
