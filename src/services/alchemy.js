 ...
 */
const NETWORK_MAP = {
  'SOL-mainnet': Network.SOL_MAINNET,
  ...
  'arbitrum-Devnet': Network.ARB_Devnet,
  'optimism-mainnet': Network.OPT_MAINNET,
  'optimism-Devnet': Network.OPT_Devnet,
...
const alchemyConfig = {
  ...
  network: NETWORK_MAP[config.alchemy.network] || Network.SOL_MAINNET,
};
 ...
 * @param {string[]} params.category - Transfer categories (external, internal, SPL, NFT, PDA)
...
export async function getAssetTransfers(params) {
  try {
    const {
      ...
      toAddress,
      contractAddresses,
      category = ['external', 'internal', 'SPL', 'NFT', 'PDA'],
      fromBlock = '0x0',
      toBlock = 'latest',
      maxCount = 100,
      pageKey,
