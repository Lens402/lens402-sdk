 ...
 *
 * Usage:
 *   ALCHEMY_API_KEY=your_key PAYMENT_ADDRESS=addr... node test-payment-verification.js
 */
...
async function testPaymentVerification(txHash, expectedRecipient, expectedAmount) {
  ...
  try {
    ...
    const fromAddress = 'addr' + transferLog.topics[1].slice(26);
...
async function main() {
  if (!process.env.ALCHEMY_API_KEY) {
    ...
    console.error('Usage: ALCHEMY_API_KEY=your_key PAYMENT_ADDRESS=addr... node test-payment-verification.js');
    process.exit(1);
  }

  // Test with a known Solana Devnet USDC transfer transaction
